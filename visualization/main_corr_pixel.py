...
def main_corr_pixel(metric, list_algorithm):
    """main function to evaluate the effect of ROI area on ROI performance.
    Parameters
    ----------
    ...
    # data initialization.
    data_pixel_avg = np.zeros(shape=[28])
    for i_attendant in list_attendant:
        ...
        for roi_tmp in np.unique(df_pixel['ROI'].values).tolist():
            data_pixel.append(df_pixel.loc[df_pixel['ROI'].values==roi_tmp, 'num_pixel'].mean())
        data_pixel_avg = data_pixel_avg + np.array(data_pixel)
    # take average over all selected rPPG algorithms.
    data_pixel_avg = data_pixel_avg/len(list_attendant)
    # visualization and save data.
    plt.cla()
    plt.xticks(fontsize=17)
    plt.yticks(fontsize=17)
    sns.regplot(x='num_pixel', 
                y=metric, 
                data=pd.DataFrame(data=np.stack([data_pixel_avg, data_evaluation_avg], axis=1), 
                                  columns=['num_pixel', metric]),
                scatter_kws={"color": "purple"}, 
                line_kws={"color": "black"},
                label=None
                )
    dir_save = os.path.join(dir_crt, 'plot', 'corr', 'num_pixel_'+metric+'.png')
    plt.savefig(dir_save, dpi=600, bbox_inches='tight')
    # regression analysis.
    X = sm.add_constant(data_pixel_avg)
    model = sm.OLS(data_evaluation_avg, X)
    result = model.fit()
    # write description file.
    with open(os.path.join(dir_crt, 'plot', 'corr', 'description_num_pixel.txt'), 'a') as f:
        f.writelines([metric, '\n', 
                      'Pearson Correlation Coefficient: '+str(np.corrcoef(data_pixel_avg, data_evaluation_avg)[0, 1]), '\n', 
...
if __name__ == "__main__":
    ...
    for metric in list_metric:
        print([metric])
        main_corr_pixel(metric=metric, list_algorithm=list_algorithm)